...
describe("Pharmacy", () => {
  ...
  it("should increase the benefit twice as fast and decrease expiresIn when Herbal Tea expired for a long time", () => {
    expect(
      new Pharmacy([new Drug("Herbal Tea", -4, 5)]).updateBenefitValue()
    ).toEqual([new Drug("Herbal Tea", -5, 7)]);
  });
  ...
  it("should not decrease the benefit or expiresIn of Magic Pill", () => {
    expect(
      new Pharmacy([new Drug("Magic Pill", 10, 30)]).updateBenefitValue()
    ).toEqual([new Drug("Magic Pill", 10, 30)]);
  });
  ...
  it("should increase the benefit by 3 and decrease expiresIn when Fervex is less than 5 days before expiration", () => {
    expect(
      new Pharmacy([new Drug("Fervex", 2, 5)]).updateBenefitValue()
  ...
  it("should decrease the benefit to 0 and decrease expiresIn when Fervex is expired for a few days", () => {
    expect(
      new Pharmacy([new Drug("Fervex", -4, 35)]).updateBenefitValue()
    ).toEqual([new Drug("Fervex", -5, 0)]);
  });

  it("should decrease by 2 the benefit and expiresIn of Dafalgan", () => {
    expect(
      new Pharmacy([new Drug("Dafalgan", 2, 3)]).updateBenefitValue()
    ).toEqual([new Drug("Dafalgan", 1, 1)]);
  });

  it("should decrease by 2 the benefit and expiresIn of Dafalgan when about to expire", () => {
    expect(
      new Pharmacy([new Drug("Dafalgan", 1, 2)]).updateBenefitValue()
    ).toEqual([new Drug("Dafalgan", 0, 0)]);
  });

  it("should decrease by 4 the benefit of Dafalgan when expired", () => {
    expect(
      new Pharmacy([new Drug("Dafalgan", 0, 15)]).updateBenefitValue()
    ).toEqual([new Drug("Dafalgan", -1, 11)]);
  });

  it("should decrease by 4 the benefit of Dafalgan when expired for a few days", () => {
    expect(
