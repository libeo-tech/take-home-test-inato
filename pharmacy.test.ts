...
describe("Herbal tea guidelines", () => {
  it("should increase benefit and decrease expiresIn", () => {
    expect(
      new Pharmacy([new Drug(drugNames.herbalTea, 1, 1)]).updateBenefitValue()
    ).toEqual([new Drug(drugNames.herbalTea, 0, 2)]);
  });
...
describe("Magic pill guidelines", () => {
  it("should never decrease benefit nor expiration date", () => {
    expect(
      new Pharmacy([new Drug(drugNames.magicPill, 1, 1)]).updateBenefitValue()
    ).toEqual([new Drug(drugNames.magicPill, 1, 1)]);
  });
});
...
describe("Fervex guidelines", () => {
  it("should increase benefit when expiration date is in more than 10 days", () => {
    expect(
      new Pharmacy([new Drug(drugNames.fervex, 14, 2)]).updateBenefitValue()
    ).toEqual([new Drug(drugNames.fervex, 13, 3)]);
  });

  it("should increase benefit by 2 when expiration date is in less than 10 days and more than 5 days", () => {
    expect(
      new Pharmacy([new Drug(drugNames.fervex, 8, 2)]).updateBenefitValue()
    ).toEqual([new Drug(drugNames.fervex, 7, 4)]);
  });

  it("should increase benefit by 3 when expiration date is in less than 5 days", () => {
    expect(
      new Pharmacy([new Drug(drugNames.fervex, 3, 6)]).updateBenefitValue()
    ).toEqual([new Drug(drugNames.fervex, 2, 9)]);
  });
